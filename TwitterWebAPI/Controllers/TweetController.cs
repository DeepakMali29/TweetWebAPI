...
namespace TwitterWebAPI.Controllers
{
    ...
    public class TweetController : Controller
    {
        ...
        public async Task<IActionResult> AddLike(string userName, int tweetId)
        {
            var response = await _TweetService.AddTweetLike(userName, tweetId);
            if (!response.Success)
            {
        ...
        [HttpPost("{userName}/reply")]
        public async Task<IActionResult> AddTweetReply(string userName, int tweetId, bool intitalComment, string message)
        {
            var response = await _TweetService.AddTweetReply(userName, tweetId, intitalComment, message);
            if (!response.Success)
            {
                return BadRequest(response);
            }
